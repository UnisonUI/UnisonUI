package tech.unisonui.providers.webook.routes
...
import tech.unisonui.providers.webhook.models.{Service => WebhookService}
...
class ServicesSpec
  ...
  private def createQueue
      : (SourceQueueWithComplete[ServiceEvent], TestProbe) = {
    val probe = TestProbe()
    val queue =
      Source
        .queue[ServiceEvent](0, OverflowStrategy.backpressure)
        .toMat(Sink.actorRef(probe.ref, "completed", _ => ()))(Keep.left)
        .run()
    queue -> probe
  }
