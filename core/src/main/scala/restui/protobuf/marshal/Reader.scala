  ...
  private def read(
    ...
    messageSchema match {
      case Some(messageSchema) =>
        for {
          result <- decodeInput(input, messageSchema).toVector.traverse {
            case (key, json) => json.map(key -> _)
          }
...
// $COVERAGE-ON$
  ...
  private def decodeInput(input: CodedInputStream,
                          ...
                          fields: Map[String, Either[Throwable, Json]] =
                            Map.empty): Map[String, Either[Throwable, Json]] =
...
// $COVERAGE-ON$
      ...
      case Type.ENUM =>
        Json
          .fromString(schema.enums(field.schema.get).values(in.readEnum()))
          .asRight[Throwable]
      case Type.MESSAGE =>
