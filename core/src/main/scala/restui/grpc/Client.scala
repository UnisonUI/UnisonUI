...
class Client(service: Service, settings: GrpcClientSettings)(implicit sys: ClassicActorSystemProvider) {
  ...
  def request(methodName: String, input: Json): Option[Future[Json]] =
    service.methods.find(m => m.name == methodName).map { method =>
      ...
      new ScalaUnaryRequestBuilder(descriptor, clientState.internalChannel, options, settings).invoke(input)
    }
  ...
  private def methodDescriptor(method: Method): MethodDescriptor[Json, Json] =
    MethodDescriptor
      ...
      .setFullMethodName(MethodDescriptor.generateFullMethodName(service.fullName, method.name))
